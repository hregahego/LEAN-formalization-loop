...
How much of the formalization is done, and whether it has stalled, lives in
signals.py — that is the loop's concern, not shared plumbing.
...
# disabled guard rather than a visible error. `MARKDOWN_LEAD` is that tolerance,
# and signals.py applies it to the `Next:` lines it reads from PROGRESS.md.
MARKDOWN_LEAD = r"[ \t>]*(?:[-*+]\s+)?\*{0,2}\s*"
_ITER_HEADER = re.compile(r"(?m)^\s*#{1,6}\s*Iteration\s+(\d+)\b")
_AGENT_LINE = re.compile(r"(?m)^" + MARKDOWN_LEAD + r"Agent\s*(\d+)\s*\*{0,2}\s*:")
_VERDICT = re.compile(r"(?mi)^" + MARKDOWN_LEAD + r"Verdict\*{0,2}\s*:\s*\*{0,2}\s*(COMPLETE|INCOMPLETE)\b")
